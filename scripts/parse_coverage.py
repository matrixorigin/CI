...
def merge_coverage_files(output_path, *coverage_files):
    ...
    logging.info(f"Starting to merge coverage files: {coverage_files}")
            ...
            with open(coverage_file, 'r') as cov_file:
                for line in cov_file:
                    ...
                    if key not in merged_coverage:
                        merged_coverage[key] = executed_count
                        total_blocks += 1
                        if executed_count > 0:
                            covered_blocks += 1
                    else:
                    ...
                    if file_name in modified_lines:
                        for modified_line in modified_lines[file_name]:
                            if start_line <= modified_line <= end_line:
                                ...
                                if executed_count > 0:
                                    covered_lines.add(modified_line)
...
if __name__ == "__main__":
    ...
    # 调用主函数
    diff_path = args.diff_path  # 你可以根据实际情况修改路径
    coverage_path = 'merged_coverage.out'  # 你可以根据实际情况修改路径
